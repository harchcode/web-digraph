...
import { createQuadTree } from "./quad-tree.ts";

describe("QuadTree", () => {
  it("should handle empty builds", () => {
    const qt = createQuadTree(100, 10, 2);
    qt.build(new Int32Array(0), (id, out) => {});
  ...
  it("should find a single item", () => {
    ...
    qt.build(new Int32Array([42]), (id, out) => {
      out[0] = 10;
      out[1] = 10;
      out[2] = 20;
      out[3] = 20;
  ...
  it("should successfully partition into 4 quadrants", () => {
    ...
    qt.build(new Int32Array([1, 2, 3, 4]), (id, out) => {
      out[0] = bboxes[id][0];
      out[1] = bboxes[id][1];
      out[2] = bboxes[id][2];
      out[3] = bboxes[id][3];
  ...
  it("should handle straddling items", () => {
    ...
    qt.build(new Int32Array([1, 2, 99]), (id, out) => {
      out[0] = bboxes[id][0];
      out[1] = bboxes[id][1];
      out[2] = bboxes[id][2];
      out[3] = bboxes[id][3];
  ...
  it("should handle large scale brute-force testing (No False Negatives)", () => {
    ...
    for (let testIdx = 0; testIdx < 10; testIdx++) {
      ...
      for (let i = 0; i < NUM_ITEMS; i++) {
        ...
        const overlaps = !(bMaxX < vX || bMinX > vMaxX || bMaxY < vY || bMinY > vMaxY);
